...
def test_webhook_progress_setting_defaults_off_and_round_trips(monkeypatch, tmp_path):
    ...
    # Рядом с `progress` в этом же наборе живёт периодическая сводка
    # (см. Api._status_report_worker) -- поля её умолчаний перечислены здесь
    # намеренно, а не выкинуты из сравнения: набор сверяется ЦЕЛИКОМ, и это
    # единственное место, которое заметит, если какое-то из них однажды
    # пропадёт или сменит умолчание.
    status_defaults = {
        "status_enabled": False,
        "status_hours": 6.0,
        "status_only_running": True,
        "status_on_stop": True,
    }
    ...
    assert object.__new__(Api).get_webhook_settings() == {
        ...
        "mention_id": "456",
        "progress": True,
        # Сводку короткий вызов не трогает -- он про неё ничего не знает.
        **status_defaults,
    }
