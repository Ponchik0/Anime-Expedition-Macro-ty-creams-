...
class Api:
    ...
    def _on_replay_silence(self, screenshot: str = None, detail: str = "") -> None:
        ...
        try:
            replay_result.send_notice(
                ...
                + (f"{detail}\n" if detail else "")
                + "Счёт ниже порога — пересними баннер своей вырезкой: Настройки → "
                "Общие → Менеджер картинок, проверить можно там же в «Проверке "
    ...
    def pop_out_wave_monitor(self) -> dict:
        ...
        win = webview.create_window(
            "Wave Monitor | Anime Expeditions",
...
def _launch_ui():
    ...
    api.push_log(f"[Macro] Anime Expeditions v{updater.get_current_version()} ({_get_build_info()}) starting...")
