...
# Ссылки на проект: Discord сообщества, исходники, YouTube автора.
# БОЛЬШЕ НЕ УХОДЯТ В WEBHOOK: блок «Links» висел на каждом результате матча
...
# ── Сторож бездействия ────────────────────────────────────────────────────
# Сколько минут «ничего не происходит» считать зависанием. Признак — строка
# текущего действия перестала меняться.
# ПОРОГ ОБЯЗАН БЫТЬ БОЛЬШЕ САМОГО ДЛИННОГО МАТЧА: во время боя действие
# подолгу не меняется, и это нормально. 25 минут с запасом перекрывают и
# Expedition, и Infinite. 0 — сторож выключен.
STALL_TIMEOUT_MIN = 25

# ── Снос позиции при расстановке ──────────────────────────────────────────
# Сканер отдаёт БЛИЖАЙШУЮ свободную клетку к заданной точке. Если она дальше
# PLACE_MAX_DRIFT пикселей — значит на самой точке клетки не было, и юнит
# встанет не туда, куда его ставили. В Expedition платформа едет и клетки
# под ней ползут, поэтому промах там обычное дело: в логах повторялся
# «offset (18, 17)» — край области поиска.
# 8px выбрано так: это заметно меньше клетки, то есть попадание считается
# точным, но допускает мелкое дрожание подсветки.
PLACE_MAX_DRIFT = 8
# Сколько раз подождать освобождения нужной клетки, прежде чем согласиться
# на дальнюю. Клетка часто освобождается сама, когда платформа доезжает.
PLACE_DRIFT_WAIT_TRIES = 3
PLACE_DRIFT_WAIT_S = 0.7

# ── Что считать подсветкой клетки, а что — мусором ────────────────────────
# ГЛАВНАЯ ПРИЧИНА «юниты не ставятся». Раньше годился ОДИН белый пиксель в
# области поиска: порог 243 по каждому каналу берёт и белый текст (имена
# юнитов, урон, номер волны), и частицы, и светлую кромку платформы. В логе
# это выглядело как повторяющийся «offset (18, 17)» — угол области поиска,
# то есть клик уходил на 25px в сторону от нужной клетки, ни во что не
# попадал, а Pre Start всё равно писал «placed» (там проверка отключена).
#
# Теперь белые пиксели собираются в связные пятна, и пятно считается
# подсветкой только если оно достаточно крупное И не похоже на штрих буквы.
# Клик идёт в ЦЕНТР пятна, а не в ближайший его пиксель, — центр клетки
# нажимается надёжнее, чем её кромка.
PLACE_HIGHLIGHT_MIN_PIXELS = 14   # пятно меньше этого — блик или частица
PLACE_HIGHLIGHT_MIN_SIDE = 4      # и в ширину, и в высоту: штрих буквы 1-2px не пройдёт

# ── Проверка «клик вообще сработал?» ──────────────────────────────────────
# Стоит копейки (один снимок области 38x38) и не требует ни одного шаблона.
# Логика: пока юнит в руке, под курсором горит подсветка. Если клик
# ЗАРЕГИСТРИРОВАЛСЯ — клетка занята юнитом (подсветка на ней пропадает) либо
# режим расстановки вышел (подсветки нет вообще). Если подсветка на том же
# месте осталась — клик не прошёл, и это единственный честный признак,
# который можно получить без unit_exist.png.
PLACE_CONFIRM_ATTEMPTS = 3   # столько раз перещёлкнуть, если подсветка не ушла
PLACE_CONFIRM_SETTLE = 0.35  # дать игре дорисовать результат клика перед пересъёмкой
# Проверяется ПЯТАЧОК СТРОГО ПОД КУРСОРОМ, а не вся область поиска, и это
# принципиально:
#   • при удержанном Shift (quick-place) режим расстановки не выходит, соседние
#     клетки остаются подсвеченными -- по всей области мы бы каждый раз видели
#     белое и решали, что клик не прошёл;
#   • над поставленным юнитом появляется белая подпись, и она тоже попадает в
#     область поиска, но не под курсор.
# Занятая клетка перестаёт быть белой (её закрыл юнит либо она стала
# «нельзя ставить»), поэтому пятачок под курсором -- самый чистый признак.
PLACE_CONFIRM_PROBE_SIZE = 5    # сторона пятачка в пикселях
PLACE_CONFIRM_PROBE_RATIO = 0.6  # какая его доля должна быть белой, чтобы считать подсветку горящей

# ── Матч кончился, а баннер не распознан ──────────────────────────────────
# ДЫРА, ИЗ-ЗА КОТОРОЙ «НЕ РАБОТАЕТ ПЕРЕИГРОВКА». Конец матча определялся ровно
# по двум картинкам -- баннерам «Victory» и «Defeat». Не совпал ни один (а
# шаблоны сняты на настройках автора движка, и на другом экране/языке/качестве
# графики они совпадают не всегда) -- и другого признака конца матча не было
# вообще: макрос честно опрашивал экран ПОЛНЫЕ 30 минут
# (MATCH_RESULT_TIMEOUT), стоя на уже законченном матче, а потом объявлял
# задачу сломанной. Со стороны это ровно «игра кончилась, а переигровки нет».
#
# Второй признак -- КНОПКА «Repeat Stage». Она есть только на экране
# результата (в бою на её месте живой «Leave Stage», это другое имя поиска),
# поэтому её появление само по себе означает: матч закончился.
MATCH_END_BUTTON_NAME = "repeat_stage"
# Кнопку опрашиваем не каждый тик: поиск шаблона по всему окну не бесплатный, а
# баннеры и так ищутся дважды в секунду. Раз в 5 опросов -- это +20% к цене
# опроса вместо +50%, и полсекунды задержки на распознавание конца матча.
MATCH_END_CHECK_EVERY = 5
# Панель результата уже на экране -- значит баннер там ТОЖЕ есть, просто не
# дотянул до обычного порога. Пробуем ещё раз мягче, прежде чем сдаться и
# признать исход неизвестным: 0.80 против обычных 0.90.
MATCH_RESULT_RELAXED_THRESHOLD = 0.80
# Исход матча не определён, но матч точно кончился. Отдельное значение, а не
# «loss»: врать в статистику и в предохранитель серии поражений нельзя.
RESULT_UNKNOWN = "ended"

# ── Раунд кончился, а мы всё ещё в этапе ──────────────────────────────────
# ТАК УСТРОЕН RAID. Там после матча не появляется экран результата с «Repeat
# Stage» — раунд просто заканчивается, и на экране снова висит «Start Game»,
# то есть этап готов к следующему забегу прямо на месте. Макрос про такой
# исход не знал вообще: ждал баннер, ждал кнопку повтора, не дожидался ни
# того ни другого и через 30 минут валил задачу. Со стороны — «переигровка
# не работает».
#
# Выходить в лобби и заходить заново в этом случае НЕЛЬЗЯ: мы и так стоим где
# надо, а лишний круг через лобби стоит минуту и может промахнуться на любом
# шаге. Правильное действие — заново отработать Pre Start (расставить юнитов:
# поле между раундами очищается) и нажать Start Game, то есть просто начать
# следующий повтор задачи здесь же.
RESULT_ROUND_ENDED = "round_ended"
# Раньше этого времени от начала боя «Start Game» на экране означает не конец
# раунда, а не до конца прожатую кнопку старта. Раунд быстрее не кончается.
MATCH_END_START_GAME_MIN_S = 25.0

# Подписи фаз в итоге расстановки (_log_placement_tally). Константами, потому
# что «Pre Start» там ещё и СРАВНИВАЕТСЯ: только у этой фазы ноль поставленных
# юнитов означает пустое поле и проигранный забег.
PLACEMENT_PHASE_PRESTART = "Pre Start"
PLACEMENT_PHASE_BATTLE = "Расстановка в бою"

# ── Догоняющая расстановка: доставить в бою то, что не встало до старта ────
# ГЛАВНАЯ ПРИЧИНА, по которой юнит не встаёт, когда клетка найдена и клик
# честно нажат: НЕ ХВАТИЛО ДЕНЕГ. Стартового баланса хватает не на всех, и
# последние блоки Pre Start бьются в подсвеченную клетку впустую — ровно это
# видно в логе от 14:33 (пять юнитов встали, шестой «НЕ ВСТАЛ на (878, 433)»
# после трёх кликов; это был четвёртый DPS подряд).
#
# Выбрасывать такой блок нельзя: забег пойдёт без юнита, который тебе нужен.
# Поэтому непоставленные юниты запоминаются и доставляются УЖЕ В БОЮ, когда
# деньги накопились, — по одному за подход, между кликами Battle-блоков.
#
# Почему это безопасно повторять: догоняющая попытка заново жмёт хоткей, а
# значит проходит ту же проверку подсветки (_click_place_spot). Если юнит
# всё-таки стоял, клетка под ним уже не белая, попытка это увидит и уйдёт.
PLACE_PENDING_FIRST_WAIT_S = 8.0   # не лезть сразу: дать волне начаться и деньгам капнуть
PLACE_PENDING_RETRY_S = 12.0       # пауза между подходами к одному и тому же юниту
PLACE_PENDING_MAX_TRIES = 4        # столько подходов на юнита, потом честно сдаёмся
