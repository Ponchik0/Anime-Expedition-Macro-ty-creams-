...
class ExpeditionOps:
    ...
    def _check_expedition_wave_result(self, hwnd, stop_event: threading.Event) -> str:
        ...
        # did clear. Same reset the color engine does below.
    ...
    def _note_checkpoint_seen(self) -> None:
        ...
        if not getattr(self, "_exp_checkpoint_streak", 0):
            self._exp_checkpoint_since = time.time()
        self._exp_checkpoint_streak = getattr(self, "_exp_checkpoint_streak", 0) + 1
    ...
    def _note_checkpoint_intercepted(self) -> None:
        ...
        be read at all."""
        now = time.time()
        if getattr(self, "_exp_checkpoint_since", 0.0) and getattr(self, "_exp_clock_marked_at", 0.0):
            self._exp_checkpoint_since += now - self._exp_clock_marked_at
        if not getattr(self, "_exp_intercept_streak", 0):
            self._exp_intercept_since = now
        self._exp_intercept_streak = getattr(self, "_exp_intercept_streak", 0) + 1
    ...
    def _expedition_checkpoint_stalled(self) -> bool:
        ...
        return bool(getattr(self, "_exp_checkpoint_since", 0.0)
    ...
    def _expedition_intercepts_stalled(self) -> bool:
        ...
        return bool(getattr(self, "_exp_intercept_since", 0.0)
