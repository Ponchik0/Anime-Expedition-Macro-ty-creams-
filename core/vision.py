...
# observed benefit.
# ФОРК: диапазон расширен с +-10% до +-25%.
# Живой случай: окно игры 1152x756 (штатный размер докинга), на экране
# отчётливо видна кнопка Enter Matchmaking, а поиск её не находит — надпись
# на экране примерно на пятую часть шире эталона 136x20. При +-10% такой
# промах не отыграть, и макрос вставал на ровном месте.
# Цена расширения та же, что была: 1.0 пробуется первым и на попадании
# возвращается сразу, а весь остальной список перебирается ТОЛЬКО когда
# обычный поиск уже промахнулся. Порядок — от близких к далёким, чтобы
# правдоподобный масштаб находился раньше сомнительного.
SCALE_FACTORS = (1.0, 0.95, 1.05, 0.90, 1.10, 0.85, 1.15, 1.20, 0.80, 1.25, 0.75)
