...
# Выключены они были ровно по одной причине: апдейтер тянул релиз автора
# движка и затирал им наши правки. Теперь источник наш — и обновление
# означает «забрать то, что я сам туда выложил», а не «откатиться к чужому».
#
# Как выпустить обновление:
#   1. закоммитить изменения и запушить в main;
#   2. поднять номер в файле VERSION (например 1.0.0 -> 1.1.0);
#   3. создать на GitHub релиз с тегом, равным этому номеру.
# Макрос сверяет VERSION с последним тегом релиза — без релиза он ничего
# не увидит, одного пуша недостаточно.
