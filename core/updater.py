...
# ФОРК: обновления берутся из ТВОЕГО репозитория, а не автора движка.
...
# проверка версии ходит туда анонимно, без всяких ключей. Отдельная константа
# — не лишняя сущность, а развилка, которая уже один раз понадобилась.
#
# ЧТО БУДЕТ, ЕСЛИ ЗАКРЫТЬ ИСХОДНИКИ. Проверка версии сломается у всех сразу:
# приватному репозиторию GitHub отвечает анонимному гостю 404 (именно 404, а
# не 403, — чтобы не выдать сам факт его существования), и в журнале это
# выглядит как «репозиторий недоступен: приватный, переименован или удалён».
# Ровно это и случилось, когда репозиторий побывал закрытым.
#
# Токеном это не лечится: токен, положенный в приложение, достаётся из него за
# минуту, то есть раздаёт доступ к приватному репозиторию всем, у кого есть
# сборка. Приватный репозиторий с зашитым токеном — это публичный репозиторий,
# только с лишними шагами.
#
# ЕСЛИ ИСХОДНИКИ ВСЁ-ТАКИ ЗАКРОЮТ, работающий путь один: завести маленький
# ПУБЛИЧНЫЙ репозиторий, в котором нет кода, а есть только релизы с
# приложенным архивом сборки, и вписать его сюда. Всё сетевое отсюда ходит
# через эту константу, так что правка ровно одна — плюс публикация релиза
# туда (release.yml, вход `repository` у softprops/action-gh-release, токен с
# правом Contents: write на тот репозиторий, потому что встроенного
# GITHUB_TOKEN хватает только на свой). Обновление ИЗ ИСХОДНИКОВ в такой схеме
# станет невозможным и честно откажется — см.
# _refuse_source_update_from_releases_repo.
...
# Обновления ВКЛЮЧЕНЫ и берутся из нашего публичного репозитория релизов
# (см. RELEASES_REPO). Выключены они были ровно по одной причине: апдейтер
# тянул релиз автора движка и затирал им наши правки. Теперь источник наш — и
# обновление означает «забрать то, что я сам туда выложил», а не «откатиться
# к чужому».
#
# Как выпустить обновление:
#   1. закоммитить изменения и запушить в main;
#   2. поднять номер в файле VERSION (например 1.1.0 -> 1.1.1);
#   3. поставить тег ровно с этим номером и запушить его:
#        git tag -a v1.1.1 -m "что нового"
#        git push origin v1.1.1
#      Текст тега станет описанием релиза (см. release.yml), поэтому писать
#      его стоит для людей, а не «Release v1.1.1».
# Дальше release.yml соберёт сборку и опубликует релиз в RELEASES_REPO сам.
#
# НОМЕР В VERSION И НОМЕР ТЕГА ДОЛЖНЫ СОВПАДАТЬ. VERSION — это «что стоит у
# меня», тег — «что вышло», и сравнивает макрос именно их. Тег НИЖЕ VERSION
# (живой случай: тег v1.0.1 при VERSION 1.1.0) читается как «у тебя уже
# новее» — проверка ответит «последняя версия», и обновление не предложится
# никому. Название релиза на это не влияет вообще: оно просто текст.
...
def _refuse_source_update_from_releases_repo() -> None:
    """Молчит, пока код и релизы в одном репозитории, — то есть сейчас всегда.

    Срабатывает, только если RELEASES_REPO разведут с GITHUB_REPO (см. их
    комментарий: так придётся сделать, если исходники закроют). Обновиться «из
    исходников» в такой схеме нельзя: клиент ходит только в публичный
    репозиторий релизов, а там архив сборки, а не наш код.

    Без этой проверки кнопка в тот день отработала бы «успешно» и не сделала
    ничего: скачала бы zip публичного репозитория (README и больше ничего),
    разложила его поверх установки, перезапустила приложение — и версия
    осталась бы прежней. Молчаливая пустая работа выглядит как поломка
    апдейтера и ищется дольше, чем честный отказ с объяснением."""
    if RELEASES_REPO != GITHUB_REPO:
        raise RuntimeError(
            "Обновление из исходников недоступно: релизы лежат в отдельном "
            f"публичном репозитории ({RELEASES_REPO}), а исходники — в "
            f"{GITHUB_REPO}, и скачать их оттуда нечем. Запуск из исходников "
            "обновляется через `git pull`. Кнопка обновления работает для "
            "собранной сборки (.exe).")
...
def stage_source_update(zip_url: str, app_dir: str, log, on_progress=None) -> str:
    ...
    _uncommitted_changes. Refuses just as flatly when releases live in their
    own repository -- there is no source to download then, only a build (see
    _refuse_source_update_from_releases_repo).
    """
    _refuse_if_disabled()
    _refuse_source_update_from_releases_repo()
